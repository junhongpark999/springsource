...
public class HomeController {
	...
	//1) rttr.addAttribute("age",10):주소줄에 age라는 이름으로 10을 보내는 방식
	//   http:// ~~~~~~~~~ ?age=10
	//	 path += "?page=+page+&amount="+amount...
	
	//2) rttr.addFlashAttribute("num","15"); 세션객체(일회성)에 담는 방식
	
	
	@GetMapping("/doB")
	public String doB(RedirectAttributes rttr) {
		rttr.addAttribute("age",10);
		rttr.addAttribute("addr","마드리드");
		rttr.addAttribute("name","Ronaldo");
		
		rttr.addFlashAttribute("num","15");
		return "redirect:/";
	}
	
	//예전 방식  (모델과 뷰를 같이 지정하는 방식)
	@GetMapping("/doC")
